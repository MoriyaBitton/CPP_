...
TEST_CASE("TEST binary on ints simple")
{
    ...
    for (int i = 0; i < 10; i++)
    {
        ...
        transform(inorder.begin(), inorder.end(), inorder.begin(), [](int val)
                  { return val + 1; });
        transform(preorder.begin(), preorder.end(), preorder.begin(), [](int val)
                  { return val + 1; });
        transform(postorder.begin(), postorder.end(), postorder.begin(), [](int val)
                  { return val + 1; });
    }
}
    ...
    for (int i = 0; i < 1; i++)
    {
        ...
        for (unsigned int k = 1; k < randomVals.size(); k++)
        {
            if (RandomBool())
            {
                ...
                std::cout << randomVals[k - 1] << " left " << randomVals[k] << std::endl;
            }
            else
            {
                tree.add_right(randomVals[k - 1], randomVals[k]);
                std::cout << randomVals[k - 1] << " right " << randomVals[k] << std::endl;
            }
        }
